...
public class GooglePlacesOAuthConnector implements OAuthConnectorInterface
{
	 ...
	 * @param config
	 */
	GooglePlacesOAuthConnector(Properties config) 
	{
		this.config = config;
		...
		try
		{
			...
			oauth_client_id    = URLEncoder.encode(config.getProperty("oauth_client_id", "OAUTH_CLIENT_ID_HERE"), ENCODING);
			oauth_redirect_uri = URLEncoder.encode(config.getProperty("oauth_redirect_uri"), ENCODING);
			oauth_scope        = URLEncoder.encode(config.getProperty("oauth_scope"), ENCODING);
